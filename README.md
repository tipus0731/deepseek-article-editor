...
- 🤖 **模型选择**：deepseek-v4-flash（快速）/ deepseek-v4-pro（深度思考，可查看思考过程）
...
- 🔍 **查重复重率**：与 [文皮皮·河图引擎](http://www.wenpipi.com/sim) **完全一致的算法**（逐字符 diff 编辑距离全文相似度，相似度 = 100 - 编辑距离/较长文长 × 100，可为负值，保留原文不截断；支持一挡/二挡/三挡切换，默认一挡与文皮皮页面默认一致；目标 ≤5%，超标自动降重重写；判重按「纯文本内容」计算（原文与 Word 正文两侧都**删除** [图片]/【图片】及表情占位符（如[捂脸][流泪]）、剔除事实核查表、统一折叠换行与空白），因此界面/文件名上的重复率与你把「发布形态原文（无占位符）+ Word 正文纯文本」粘到文皮皮·河图引擎检测的结果一致，不受段落换行/表情占位/版面差异影响）
...
- **deepseek-v4-pro 较慢**：深度思考模型会先推理再输出，属正常现象；需要快速出稿请选 deepseek-v4-flash。
...
- **下载变保存**：「下载 .txt / 图片」通过 JS 桥保存到手机 **相册/文章助手**（图片）与 **下载/文章助手**（Word/txt 文档，Android 10+ 免权限，9 及以下需存储权限）
- 「原图」在新窗口打开时自动跳转系统浏览器
