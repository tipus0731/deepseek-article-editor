...
- 🔍 **查重复重率**：与 [文皮皮·河图引擎](http://www.wenpipi.com/sim) **完全一致的算法**（逐字符 diff 编辑距离全文相似度，相似度 = 100 - 编辑距离/较长文长 × 100，可为负值，保留原文不截断；支持一挡/二挡/三挡切换，默认一挡与文皮皮页面默认一致；目标 ≤5%，超标自动降重重写；判重以「导出 Word 中的实际正文」为准（剔除事实核查表、去图片标记与空行），因此界面/文件名上的重复率与把导出 Word 正文粘到文皮皮·河图引擎检测的结果一致）
